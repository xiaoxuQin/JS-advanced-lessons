/*              高阶函数               */
// part 01 函数作为参数被传递（最常见的形式：回调函数）
// e.g. 01  高阶函数的一般应用
function add(x, y, f){
    return f(x) + f(y);
}
add(2, 3, function(z){return z*z;});    // 13  平方
add(2, -3, Math.abs);       // 5  绝对值
add(2, -3, Math.sqrt);      // NaN  开方
add(1, 9, Math.sqrt);       // 4
...
// e.g. 03 下述实例本章仅供参考，详细内容在Array章节具体介绍
// 数组相关的高阶函数 map reduce filter sort详情参见数组章节
function pow(x) {
    return x * x;
}
...
for (var i=0;i<result.length;i++){
    console.log(typeof result[i]);  // number number number
}
