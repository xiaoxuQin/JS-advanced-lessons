...
// (CSDN)
// Object.isExtensible()方法确定对象是否可扩展（是否可以添加新属性）。
// 新对象默认是可扩展的，无论何种方式创建的对象，这里使用的是字面量方式
var empty1 = { a: 1 };
...
//      Object.isExtensible 和 Object.preventExtensions实例     
...
(function () {
    ...
    // 使用 Object.defineProperty方法 为不可扩展对象添加新属性会抛出异常
...
// 如果对象被密封则返回 true ，否则返回 false
// 如果对象是不可扩展的, 并且其所有属性都是不可配置的
// 并且因此不可移除（但不一定是不可写的），则该对象被密封
(function () {
    // 新建的对象默认不是密封的
    var empty = {};
    console.log(Object.isSealed(empty)); // false

    // 如果  !!! 把一个空对象变得 * 不可扩展 * ,则它同时也会变成个密封对象.
    Object.preventExtensions(empty);
...
(function () {
    ...
    // 但不能把密封对象的属性进行 重新配置,譬如将 数据属性 重定义成 访问器属性
...
// 且所有属性的 可写特性 为false则该对象为密封的对象
// (CSDN)
//  *当且仅当* 对象不可扩展时，对象才会被冻结，其所有属性都是不可配置的，
// 并且其所有数据属性（即，不具有getter或setter组件的访问器属性的属性）都是不可写的
(function () {
    // 一个对象默认是可扩展的,所以他也是非冻结的.
    console.log(Object.isFrozen({}));   // false

    // 一个不可扩展的 *空对象* 同时也是一个冻结对象. 一个不可扩展的空对象也是密封对象
